...
class SGDPolyDualMountainCarSolver:
    def __init__(self, n_episodes=1000, max_env_steps=None, gamma=0.9, epsilon=1.0, epsilon_min=0.01,
                 epsilon_decay=0.005, alpha=0.0001, batch_size=32, c=10, monitor=False, cutoff=2000):
        ...
        self.feature_tuning.fit(np.reshape(np.hstack((self.env.reset(), 0)), [1, 3]))
        # Initialize model
    ...
    def preprocess_state(self, state, action):
        ...
        poly_state = self.feature_tuning.transform(np.reshape(np.hstack((state, action)), [1, 3]))
        return poly_state
...
if __name__ == '__main__':
    ...
    agent = SGDPolyDualMountainCarSolver()
    agent.run()
