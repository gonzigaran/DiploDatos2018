...
class DQNMountainCarSolver:
    def __init__(self, n_episodes=1000, max_env_steps=None, gamma=0.99, 
                 epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.005, 
                 alpha=0.01, alpha_decay=0.01, batch_size=32, c=10, 
                 monitor=False, n_neurons=[24,48], 
                 activations=['relu','relu'], cutoff=2000):
        ...
        assert len(self.n_neurons) == len(self.activations)
        ...
        for i in range(self.n_layers):
            if i==0:
                self.model.add(Dense(self.n_neurons[i], 
                                     input_dim=2,
                                     activation=self.activations[i]))
            else:
                self.model.add(Dense(self.n_neurons[i], 
                                     activation=self.activations[i]))
        self.model.add(Dense(3, activation='linear'))  # identity activation function is set for the two outputs
    ...
    def run(self):
        ...
        for e in range(self.n_episodes):
            ...
            while not done:
                ...
                if state[0,1] > 0 and action == 2:
                    reward = 1
                if state[0,1] < 0 and action == 0:
                    reward = 1
                if state[0,0] >= 0.5:
                    reward = 1
