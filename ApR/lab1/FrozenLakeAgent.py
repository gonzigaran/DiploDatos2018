...
class FrozenLakeAgent:
    ...
    def __init__(self):
        ...
        self._policy = 'egreedy'
        self._algorithm = 'qlearning'
        self._alpha = 0.5
        self._gamma = 0.9
        self._epsilon = 0.1
        self._tau = 25
    ...
    def run(self):
        ...
        for i_episode in range(self.episodes_to_run):
            ...
            for t in range(self._cutoff_time):
                ...
                if not done:
                    action = self.learn(state, action, reward, next_state)
                    state = next_state
                else:
                    if reward == 0:  # episode finished because the agent fell into a hole
                        ...
                        reward = self._penalize  # replace this number to override the reward
    ...
    def choose_action(self, state):
        ...
        if self._policy == 'egreedy':
            ...
            return action
        elif self._policy == 'softmax':
            q = [self.q.get((state, a), 0) for a in self.actions]
            exp_q = [np.exp(x/self._tau) for x in q]
            sum_exp_q = np.sum(exp_q)
            pi = np.array([x/sum_exp_q for x in exp_q])
            pi_acum = pi.cumsum()

            i = np.searchsorted(pi_acum, self.random_state.uniform())
    ...
    def learn(self, state, action, reward, next_state):
        ...
        new_action = action
        if self._algorithm == 'qlearning':
            new_q = max([self.q.get((next_state, a), 0.0) for a in self.actions])
        elif self._algorithm == 'sarsa':
            new_action = self.choose_action(next_state)
            new_q = self.q.get((next_state, new_action), 0.0)
